...
> **[2026-07-28更新] 這節已被 `KIRO_BUILD_CHECKLIST.md`（專案根目錄）取代為執行時的權威版本**——那份文件把下面的階段拆到「每個檔案」的粒度、每個項目都要打勾+跟團隊確認才能繼續，且補上了鷹架階段才發現的 5 個架構空白、`GATEWAY` 接線問題的修正說明。這裡保留給 Kiro 看整體脈絡（規格 spec 對應到哪個階段），實際執行時以 `KIRO_BUILD_CHECKLIST.md` 為準，不要只看這裡的粗顆粒度版本就開始生成。
